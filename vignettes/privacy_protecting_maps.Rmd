...
R makes it easy to plot spatial data using packages such as `ggplot2`, 
`tmap`, `mapview` or `leaflet`. 
However when plotting the spatial distribution of a 
_sensitive_ variable, you may accidentally reveal a sensitive value of an 
individual observation, e.g. the income of a person. The procedure of adjusting
output data to protect individual observations is called 
_statistical disclosure control_ and several open source tools are available, 
including [$\tau$-Argus](http://research.cbs.nl/casc/tau.htm) and R packages [sdcTable](https://cran.r-project.org/package=sdcTable)
and [sdcMicro](https://cran.r-project.org/package=sdcMicro). Most SDC software
operates on values of records, mostly ignoring spatial properties (if any) 
of data.
...
(`min_count`) and its maximum value should not comprise 95% (`max_risk`) of its total value, 
then 64% of the data in the map is sensitive!

For enterprise data a threshold of 10 may be on the high side, so let us change that
into 5:
...
raster::plot(mean_production, col=hcl.colors(15, "YlOrRd", rev = TRUE))
```


`protect_quadtree` is another protecting method, which we demonstrate with
the other variable `fined`. 

First we create a more fine grained (punned not intended) raster for the variable
`fined`. 
...
The result is blocky in comparison with the smoothing method:
```{r}
